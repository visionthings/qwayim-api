<x-dashboard.layout title="الإعدادات والتحكم">
    ...
    @endpush
    @section('navContent')
      <h2 id="navhead" class="h1 mb-0">الإعدادت والتحكم <small> تعديل الخبر</small></h2>
    @endsection
      ...
      <div class="row">
          ...
          <div class="col-md-10">
              <div class="box shadow rounded-3 p-2">
                  ...
                  <div class="container-fluid">
                    <form action="{{route('cities.news.update',$city->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        @include('Dashboard.city.news._form',['btn_text'=>'تعديل','city'=>$city])
                    </form>
                  </div>
              </div>
          </div>
